...
static AXError TestAttribute(AXUIElementRef e, CFStringRef name, CFTypeRef *out) {
  ...
  if (e==fakeField && CFEqual(name,kAXRoleAttribute)) { *out=CFRetain(kAXTextFieldRole); return kAXErrorSuccess; }
...
int main(void) { @autoreleasepool {
  ...
  bool pasted=result==1 && posted==4 && [[testBoard stringForType:NSPasteboardTypeString] isEqualToString:@"hello from Pulse"];
  ...
  printf("focused_step=%d no_field_step=%d copied=%d\n",result,noField,copied);
