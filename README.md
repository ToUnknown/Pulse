...
- Text Extractor prototype: use Win + Shift + T to open the animated editor, or Ctrl + Win + Shift + T to select and instantly copy local OCR text. Selection happens over the live desktop. Basic uses local Windows OCR by default, with no API key. Enable it and customize either shortcut in Settings → Advanced. Adding the optional shared OpenAI API key unlocks Advanced extraction with GPT-5.6 Luna (no reasoning) and Translate; the key is checked before it is saved. Escape or an outside click dismisses the overlay.
