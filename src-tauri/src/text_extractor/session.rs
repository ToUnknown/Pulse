...
fn advanced_available() -> bool {
    openai_credentials::is_configured().unwrap_or(false)
}
...
fn load_preferences(bytes: &[u8]) -> Result<Preferences, String> {
    ...
    let removed_subscription_provider =
        value.get("advancedProvider").and_then(Value::as_str) == Some("codex");
    let mut preferences: Preferences =
        serde_json::from_value(value).map_err(|_| "Could not read Text Extractor settings.")?;
    // Do not silently send captures to API billing when an old install selected
    // the removed subscription option. Let the user opt into Advanced again.
    if removed_subscription_provider {
        preferences.editor_mode = ExtractionMode::Basic;
        preferences.quick_mode = ExtractionMode::Basic;
    }
...
pub async fn text_extractor_quick_copy(
    ...
    let recognized = tokio::select! {
        ...
        result = async {
            ...
            // Keep API-key availability checks off the shortcut-to-selector path.
            // Basic remains usable when Advanced is unavailable.
            let mode = default_mode.with_advanced_available(advanced_available());
    ...
    let result = tokio::select! {
        ...
        result = async {
            if use_advanced.unwrap_or(false) {
                request_advanced(protocol::translation_body(&text, &language)?, cancel.clone()).await
            } else {
...
async fn recognize_selection(
    ...
    match mode {
        ...
        ExtractionMode::Advanced => {
            ...
            request_advanced(protocol::request_body(&image_url), cancel).await
        }
    }
}
...
mod tests {
    ...
    fn removed_subscription_selection_resets_advanced_modes() {
        ...
        assert!(saved.get("advancedProvider").is_none());
    }
}
