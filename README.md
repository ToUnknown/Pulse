...
- Dictation: enable **Tap or hold to dictate** in Settings → Advanced, add your shared OpenAI API key, and allow Microphone and Accessibility access. Tap **Right Option** to stream speech to `gpt-live-transcribe` hands-free; press it again to finish. Alternatively, hold it for at least half a second, then release to finish. Recording starts immediately on key-down. The small bottom waveform pill expands into a compact transcript and scrolling waveform, then collapses into a loading circle and fades away after delivery. There is no success badge or flying text. Accessibility is required to detect the modifier-only shortcut and identify the selected input. Left Option does not start dictation; using Right Option with another key during a hold cancels dictation and keeps the normal key combination working. If no editable field is selected, the transcript is copied to the clipboard. Escape cancels. If focus or the selection changes during delivery, Pulse copies instead. Delivery uses the standard macOS paste command so web editors receive normal paste events, and leaves a copy on the clipboard. Unsupported fields fall back to clipboard. Recordings are limited to five minutes; audio is buffered in memory, never saved to disk, and sent only during a dictation session. A failed connection preserves available partial text on the clipboard and reports the failure. The last completed or partial transcript stays in memory until Pulse quits and can be copied again from Settings. Dictation currently ships on macOS only; its planned Windows equivalent is Right Alt.
