...
- Dictation (macOS): enable **Tap or hold to dictate** in Settings → Advanced, add the shared OpenAI API key, and allow Microphone and Accessibility access. Tap **Right Option** to start hands-free recording and press again to finish, or hold for at least half a second and release. Recording starts immediately. A compact 64×28 native clear-glass pill follows the insertion point with smooth movement and a voice-reactive waveform. Caret geometry follows wrapping and growing fields where the app exposes it; otherwise the field bounds are used. Supported fields receive live Unicode input with minimal corrections and verified text/selection read-back, without reading or changing the clipboard. Clicking away moves the pill to bottom center with a small live transcript above it. Pulse restores the old field's pre-dictation text through a verified accessibility edit before adopting the next selected input, which receives the complete transcript and continues live insertion. Handoffs never activate or raise windows. Recreated controls require matching stable identity, parent, window, and process to retain their existing owned range. If the current field is safely selected and synchronized at completion, final corrections stay there and the clipboard is untouched. Otherwise the complete transcript is copied once. If an app refuses background accessibility edits or the field was independently changed, Pulse leaves that content intact rather than risk deleting user edits. Unsupported or changed fields quietly use the same preview/clipboard path. The overlay shows no success or error messages; operational diagnostics and the last transcript remain in Settings. Escape cancels without copying, leaving already inserted text intact. Left Option does not activate dictation; a Right Option keyboard chord cancels a held recording. Recordings are limited to five minutes, with audio buffered in memory and sent to `gpt-live-transcribe` only during a session. Text remains in memory until Pulse quits and can be explicitly copied from Settings. Dictation currently ships on macOS only; its planned Windows equivalent is Right Alt.
