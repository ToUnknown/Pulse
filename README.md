...
- Dictation: enable **Hold to dictate** in Settings → Advanced, add your shared OpenAI API key, and allow Microphone access. Hold **Option + Shift + Space** to stream speech to `gpt-live-transcribe`; release to finalize. The bottom waveform pill expands into live text and a scrolling waveform, then collapses into a loading circle while the transcript travels to the selected input. Allow Accessibility for insertion and caret positioning; otherwise the transcript is copied to the clipboard. Escape cancels. If focus or the selection changes during delivery, Pulse copies instead. Apps without caret bounds use the editable field bounds; unsupported fields fall back to clipboard. Paste-based insertion also leaves a copy on the clipboard. Recordings are limited to five minutes; audio is buffered in memory, never saved to disk, and sent only during a dictation session. A failed connection preserves available partial text on the clipboard and reports the failure. The last completed or partial transcript stays in memory until Pulse quits and can be copied again from Settings. Dictation currently ships on macOS only.
