...
- Audio assistant (Windows only, off by default): Ctrl + Alt + Space toggles system-audio listening; Ctrl + Shift + Space answers the latest question in a small top-left overlay. Both shortcuts are editable in Settings → Advanced. Listening requires the shared OpenAI API key, including when Codex is selected for answers. Audio is streamed once to OpenAI transcription; only a rolling 30-second transcript stays in memory. Turning listening off cancels capture, transcription, and answers and clears that transcript. Optional UTF-8 `.txt` / `.md` context files are limited to 32 KB. Answers reuse the Advanced provider selection: available Codex Luna with its lowest supported reasoning, otherwise OpenAI Luna with reasoning disabled. Repeated answer requests replace the previous answer; Escape dismisses the overlay. Listening preference and shortcuts survive restarts. No transcript or summary is saved.
