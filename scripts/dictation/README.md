...
There is no field locking, live editor mutation, or transcript history. After recording ends and modifiers are released, delivery copies the complete transcript to the clipboard and sends one platform Paste shortcut to the active app. If the active app/window or physical modifiers change while copying, Pulse leaves the transcript on the clipboard without sending Paste. If another app replaces the clipboard before Paste, Pulse skips the shortcut so it cannot insert that app's content. A failed clipboard write is retried once; if both writes fail, the bottom overlay shows an error. macOS resolves the Paste key through the active keyboard layout. The clipboard retains the transcript even if the app rejects Paste. Pulse cannot guarantee acceptance by every custom editor or elevated Windows app, and it does not retry the shortcut because that could insert duplicate text.
